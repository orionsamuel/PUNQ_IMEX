...
c
	...
	real inund, lixo(977,977)
	integer cont, modelo
	REAL(4) tempo_cpu, flixo(977)
	cont=0
	it=1
      write(k0,995)it
      kode=0
...
c
	...
	open(73,file='C:\temp_flavio001\INUND.DAT',status='unknown')
c
...
945	if(HOTSTART_DAT_FLAG) then
	...
	do iii=1,k
		...
		write(73,1050) (x_inund(n_inund,jj),jj=1,n)
		endif
		endif
	enddo
...
c	cont=cont+1
c	if(cont.EQ.3)then
c	OPEN(unit=32,FILE='C:\temp_flavio001\modelo_preso.dat',
c	1status='unknown')
c	write(32,*) modelo
c	write(*,*) 'MODELO NO QUAL FIQUEI PRESO', modelo
c	
c	WRITE(*,*) '****MODELO A SER TROCADO**** ', modelo
c	open(unit=31,file='c:\temp_flavio001\HOTSTART_CAIXA40.DAT',
c	1status='unknown')
c	
c		do ii=1,k
c		read(31,*) iii
c			if(iii.EQ.modelo)then
c									
c				read(31,*) (x(modelo,j),j=1,n)
c				
c			else
c				read(31,*) (lixo(iii,j),j=1,n)
c			endif
c			
c		enddo
c			
...
c		enddo
c	
cc		write(*,*)'Leitura do arquivo hot start--OK'
c	cont=0
c	endif
c		close(31)
...
17    format(f10.4, 100(1x,f5.0))
	...
	open(34,file='C:\temp_flavio001\IMEDIATO_FO.DAT',STATUS='unknown')
	open(35,file='C:\temp_flavio001\IMEDIATO.DAT',STATUS='unknown')
	write(34,*) iev2 , it , abs(f(iev2)) , iev1 , abs(f(iev1))
	
	write(35,*) it, 'iteracao'	
	write(35,*) iev2 , iev1
	do hh=1, n
		write(35,*) x(iev2,hh) , x(iev1,hh)
	enddo
	close(35)
...
C	write(*,*)'****Escrevendo no arquivo HOTSTART.DAT*****'
C	open(unit=30,file='c:\temp_flavio001\HOTSTART.DAT',
C	1status='unknown')
C		write(30,9993) it
C9993		format(i4,1x,'Iteracao')    
C		do ii=1,k
C			write(30,*) ii
C			write(30,9991) (x(ii,j), j=1,n)
C9992			format(f10.4)
C	enddo
C	do ii=1,k
C			write(30,*) ii
...
c
      do 50 j=1,n
         write(25,*) i, x(i,j), delta, g(j), h(j)
